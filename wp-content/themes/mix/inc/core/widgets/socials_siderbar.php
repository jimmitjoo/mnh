...
// Widget Backend
    ...
    <div class="wpo_socials_siderbar">
        ...
        <p>
            <label for="<?php echo $this->get_field_id('position'); ?>">Position Share:</label>
            </br>
            <select name="<?php echo $this->get_field_name( 'position' ); ?>" id="<?php echo $this->get_field_id( 'position' ); ?>">
            <?php foreach($positions as $val => $pos){ ?>
                <option value="<?php echo $val; ?>" <?php selected( $instance['position'], $val ); ?>><?php echo $pos; ?></option>
            <?php } ?>
            </select>
        </p>

        <p>
            <label for="<?php echo $this->get_field_id( 'list_service' ); ?>"><?php echo __( 'Share services social:', TEXTDOMAIN ); ?></label>
            <br>
            <select multiple name="<?php echo $this->get_field_name( 'list_service' ); ?>[]" id="<?php echo $this->get_field_id( 'list_service' ); ?>" style="width:100%;height:200px;">
               <?php foreach( $services as $value ){ ?>
                <?php
                    $selected = ( in_array($value['val'], $instance['list_service'] ) )?' selected="selected"':'';
                ?>

                <option value="<?php echo $value['val']; ?>" <?php echo $selected; ?>>
                    <?php echo $value['text']; ?>
                </option>
               <?php } ?>
            </select>
        ...
        <p>
            <label for="<?php echo $this->get_field_id('skin'); ?>">Theme:</label>
            </br>
            <select name="<?php echo $this->get_field_name( 'skin' ); ?>" id="<?php echo $this->get_field_id( 'skin' ); ?>">
            <?php foreach($themes as $skin=>$theme){ ?>
                <option value="<?php echo $skin; ?>" <?php selected( $instance['skin'], $skin ); ?>><?php echo $theme; ?></option>
            <?php } ?>
            </select>
        </p>

        <p>
            <label for="<?php echo $this->get_field_id('show_mobile'); ?>">Show moblie:</label>
            </br>
            <select name="<?php echo $this->get_field_name( 'show_mobile' ); ?>" id="<?php echo $this->get_field_id( 'show_mobile' ); ?>">
            <?php foreach($shows_mobile as $k => $vl){ ?>
                <option value="<?php echo $k; ?>" <?php selected( $instance['show_mobile'], $k ); ?>><?php echo $vl; ?></option>
            <?php } ?>
            </select>
        </p>
    </div>
